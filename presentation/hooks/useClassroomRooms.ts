...
export const useClassroomRooms = (classId: string | null, hasSupabase: boolean) => {
  ...
  useEffect(() => {
    const fetchAppUserId = async () => {
      ...
      try {
        ...
        console.error("Error fetching app user id:", err);
      }
    };
    if (hasSupabase) fetchAppUserId();
  }, [supabase, hasSupabase]);

  const canManageRooms = isAdministrator || isInstitutionManager || isTeacher;
  ...
  }, [roomUseCase, classId, isAdministrator, showDeleted]);
  ...
  const restoreRoom = async (id: string) => {
    if (!roomUseCase || !isAdministrator) return;
